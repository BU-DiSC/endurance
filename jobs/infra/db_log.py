...
def log_new_run(
    ...
    cursor.execute(
        ...
        INSERT INTO runs (
            ...
            range_queries,
            writes,
            max_bits_per_element,
            physical_entries_per_page,
            range_selectivity,
            entries_per_page,
            total_elements,
...
def log_design_cost(
    ...
    k_values = design.K + [None] * (
        20 - len(design.K)
    )  # TODO replace this with the max_levels
    sql_command = (
        "INSERT INTO design_costs (run_id, bits_per_element, size_ratio, policy, Q, Y, Z, cost, "
        + ", ".join([f"k{i+1}" for i in range(20)])
        + ") "
        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?"
        ", " + ", ".join(["?"] * 20) + ")"
    )
    cursor.execute(
        sql_command,
        (run_id, design.h, design.T, policy.name, design.Q, design.Y, design.Z, cost)
        + tuple(k_values),
    )
    connector.commit()
...
def log_run_details(
    ...
    bayesian_cost: float,
    analytical_design: LSMDesign,
    bayesian_design: LSMDesign,
) -> None:
    cursor = connector.cursor()
    ...
    cursor.execute(
        ...
        (
            ...
            analytical_design.h,
            analytical_design.T,
            analytical_policy.name,
            analytical_design.Q,
            analytical_design.Y,
            analytical_design.Z,
            bayesian_design.h,
            bayesian_design.T,
            bayesian_policy.name,
            bayesian_design.Q,
            bayesian_design.Y,
            bayesian_design.Z,
        ),
    ...
    if bayesian_policy == Policy.KHybrid:
        ...
        cursor.execute(
            """
            INSERT INTO run_details_k_values (
                run_id, k1, k2, k3, k4, k5, k6, k7, k8, k9, k10,
