...
from endure.lcm.data.generator import (
    ClassicGenerator,
    QCostGenerator,
    YZCostGenerator,
    KHybridGenerator,
...
def print_best_designs(best_designs: List[Tuple[LSMDesign, float]]) -> None:
    ...
    for design, cost in sorted_designs[:1]:
        if design.policy == Policy.KHybrid:
            ...
            print(
                f"Design: h={design.h}, T={design.T}, Policy={design.policy}, K=[{k_values_str}], Cost={cost}"
            )
        else:
            print(
                f"Design: h={design.h}, T={design.T}, Policy={design.policy}, Q={design.Q}, Y={design.Y},"
    ...
    with open("best_designs.txt", "w") as file:
        ...
        for design, cost in best_designs:
            file.write(
                f"Design: h={design.h}, T={design.T}, Policy={design.policy}, Q={design.Q}, "
...
class BayesianPipeline:
    def __init__(self, config: dict) -> None:
        ...
        model_type_str = self.jconfig.get("model_type", "Classic")
    ...
    def _update_best_designs(
        ...
        for x, y in zip(new_x, new_y):
            if self.model_type == Policy.QFixed:
                ...
                best_designs.append(
                    (
                        LSMDesign(h=h.item(), T=np.ceil(size_ratio.item()), policy=pol),
                        y.item(),
                    )
                )
        return best_designs
