...
class BayesianPipeline:
    def __init__(self, config):
        ...
        self.cf = EndureCost(self.config)
        ...
        self.h_bounds = torch.tensor([self.bayesian_setting["bounds"]["h_min"],
                                      self.bayesian_setting["bounds"]["h_max"]])
        self.T_bounds = torch.tensor([self.bayesian_setting["bounds"]["T_min"],
        ...
        self.train_x, self.train_y = self._generate_initial_data(self.system, self.initial_samples)
        self.scaled_train_x, self.standardized_train_y = self._scale_and_standardize(self.train_x, self.train_y)
        self.mll, self.gp_model = self._initialize_model(self.scaled_train_x, self.standardized_train_y)

    def run(self) -> None:
        self.log.debug("Starting Bayesian Optimization")
        for i in range(self.num_iterations):
            new_x, new_y = self._optimize_acquisition_function(self.acquisition_function, self.gp_model, self.train_y)
    ...
    def _generate_initial_data(self, system: System, n: int = 30) -> Tuple[torch.Tensor, torch.Tensor]:
        train_x = []
        train_y = []
        for _ in range(n):
            ...
            cost = -self.cf.calc_cost(design, system, self.workload.z0, self.workload.z1, self.workload.q, self.workload.w)
    ...
    def _initialize_model(self, train_x: torch.Tensor, train_y: torch.Tensor, state_dict: dict = None) -> Tuple[SingleTaskGP, ExactMarginalLogLikelihood]:
    ...
    def _optimize_acquisition_function(self, acquisition_function: str, model: SingleTaskGP, target: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        new_designs = [LSMDesign(x[0].item(), np.ceil(x[1].item()), Policy.Leveling) for x in new_x]
        # the new cost is also negated here
        new_y = torch.tensor([-self.cf.calc_cost(design, self.system, self.workload.z0, self.workload.z1
                                                 , self.workload.q, self.workload.w) for design in new_designs])
        return new_x, new_y

    def _update_best_designs(self, new_x: torch.Tensor, new_y: torch.Tensor) -> None:
        for x, y in zip(new_x, new_y):
            self.best_designs.append((LSMDesign(x[0].item(), np.ceil(x[1].item()), Policy.Leveling), y.item()))
    ...
    def _find_analytical_results(self):
        ...
        cost = solver.nominal_objective(x, policy, self.system, self.workload.z0,
                                        self.workload.z1, self.workload.q, self.workload.w)
