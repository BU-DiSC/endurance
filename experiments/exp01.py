...
class Exp01():
    ...
    def get_endurek_trees(self) -> list[dict]:
        ...
        for id in self.config['exp_config']['Exp01']['wl_ids']:
            ...
            self.log.debug(f'Workload: {wl["id"]}: {{z0: {wl["z0"]}, z1: {wl["z1"]}, q: {wl["q"]}, w: {wl["w"]}}}')
            tree = problem.get_nominal_design(default_tree, wl)
            tree = tree.as_dict()
            tree.update(wl)
            trees.append(tree)
            self.log.debug(f'EndureK: h={tree["h"]:.2f}, T={tree["T"]:.2f}, Q={tree["Q"]:.2f}')
    ...
    def get_endure2_trees(self) -> list[dict]:
        ...
        for id in self.config['exp_config']['Exp01']['wl_ids']:
            ...
            tree.update(wl)
            trees.append(tree)
            self.log.debug(f'EndureK: h={tree["h"]:.2f}, T={tree["T"]:.2f}, policy={tree["policy"]}')
