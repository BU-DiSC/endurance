...
class EndureTierLevelSolver(EndureSolver):
    ...
    def robust_objective(
        ...
        h, T, lamb, eta = x
        query_cost = z0 * self.kl_div_con((self._cf.Z0(h, T, self.policy) - eta) / lamb)
        query_cost += z1 * self.kl_div_con(
            (self._cf.Z1(h, T, self.policy) - eta) / lamb
        )
        query_cost += q * self.kl_div_con((self._cf.Q(h, T, self.policy) - eta) / lamb)
        query_cost += w * self.kl_div_con((self._cf.W(h, T, self.policy) - eta) / lamb)
    ...
    def find_robust_design(
        ...
        sol = self._solve_robust(rho, z0, z1, q, w, init_args, bounds, minimizer_kwargs)
        return sol
    ...
    def find_nominal_design(
        ...
        if init_args is None:
            init_args = [H_DEFAULT, T_DEFAULT]
        bounds = self.get_bounds()
        sol = self._solve_nominal(z0, z1, q, w, init_args, bounds, minimizer_kwargs)
        return sol
...
class EndureQSolver(EndureSolver):
    ...
    def find_robust_design(
        ...
        sol = self._solve_robust(
            rho, z0, z1, q, w, init_args, bounds, minimizer_kwargs=minimizer_kwargs
        )
        return sol
    ...
    def find_nominal_design(
        ...
        sol = self._solve_nominal(
            z0, z1, q, w, init_args, bounds, minimizer_kwargs=minimizer_kwargs
        )
        return sol
...
class EndureYSolver(EndureSolver):
    ...
    def robust_objective(
        ...
        h, T, Y, lamb, eta = x
        query_cost = z0 * self.kl_div_con((self._cf.Z0(h, T, Y, self.Z) - eta) / lamb)
        query_cost += z1 * self.kl_div_con((self._cf.Z1(h, T, Y, self.Z) - eta) / lamb)
        query_cost += q * self.kl_div_con((self._cf.Q(h, T, Y, self.Z) - eta) / lamb)
        query_cost += w * self.kl_div_con((self._cf.W(h, T, Y, self.Z) - eta) / lamb)
        cost = eta + (rho * lamb) + (lamb * query_cost)
        return cost
